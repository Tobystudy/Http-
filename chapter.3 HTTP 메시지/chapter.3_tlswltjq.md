...
안전하다는 것의 의미 = 요청의 결과로 서버에 일어나는 일은 아무것도 없다는것.
### 3.3.2 GET
### 3.3.3 HEAD
...
### 3.3.4 PUT
서버에 요청의 본문을 통해 문서를 새로 만들거나 교체
### 3.3.5 POST
데이터를 서버에 보내기위해 사용 PUT은 서버에 있는 리소스에 데이터를 "입력"하기위해 사용.
### 3.3.6 TRACE
...
? : 100 Continue를 의도하지 않은 클라이언트에게 보내면 안되는 이유가 무엇일까
